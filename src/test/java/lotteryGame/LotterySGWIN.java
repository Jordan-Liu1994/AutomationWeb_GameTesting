...
public class LotterySGWIN extends VariablesStore {
	...
	private static String nameOfReport = "LotterySGWIN";

	@BeforeClass(groups = { "Start", "LotterySGWIN" })
	...
	@Test(priority = 0, groups = { "LotterySGWIN" })
	@Parameters({ "url" })
	...
	@Test(priority = 1, groups = { "LotterySGWIN" })
	...
	@Test(priority = 2, groups = { "login", "LotterySGWIN" })
	@Parameters({ "userIDFE" })
	...
	@Test(priority = 3, groups = { "LotterySGWIN" })
	@Parameters({ "lotteryVendorSGWIN", "time" })
	public void selectLotteryVendor(String lotteryVendorSGWIN, int time) throws FailedLoginException, InterruptedException {
		cR.createTest("selectLotteryVendor");
		sGCVAG.hoverGameCategory("6");
		sGCVAG.selectGameVendorOnly(lotteryVendorSGWIN, time);
	}

	@AfterMethod(groups = { "LotterySGWIN" })
	public void logCaseStatus(ITestResult result) {
		rL.logCaseStatus(result);
	}

	@AfterClass(groups = { "LotterySGWIN" })
